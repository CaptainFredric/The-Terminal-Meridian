...
export function createSettingsRenderer(context) {
  const { state, authEnabled } = context;
  ...
  return function renderSettings() {
    ...
    const accountSection = user ? `
      <div class="settings-panel-user">
        <div class="settings-avatar">${(user.first_name?.[0] || user.username?.[0] || "?").toUpperCase()}</div>
        <div class="settings-user-info">
          <strong>${user.first_name || ""} ${user.last_name || ""}`.trim() + `${!user.first_name && !user.last_name ? (user.username || "Account") : ""}</strong>
      ...
      <div class="settings-section-row">
        <button class="btn btn-sm btn-secondary" data-settings-action="edit-profile">Edit Profile</button>
        <button class="btn btn-sm btn-secondary" data-settings-action="change-password">Change Password</button>
        ${isPro ? `<button class="btn btn-sm btn-ghost" data-settings-action="manage-billing">Manage Billing</button>` : ""}
        ${!isPro ? `<button class="btn btn-sm btn-accent" data-settings-action="upgrade">Upgrade to Pro</button>` : ""}
      </div>
    ` : `
      <div class="settings-panel-guest">
        ...
        <div class="settings-section-row">
          <button class="btn btn-sm btn-accent" data-settings-action="sign-in">Sign in</button>
          <button class="btn btn-sm btn-secondary" data-settings-action="create-account">Create account</button>
        </div>
      </div>
    `;
        ...
        ${tier === "free" ? `
          ...
          <div class="settings-plan-grid">
            <div class="settings-plan-card">
              ...
              <button class="btn btn-sm btn-accent" data-settings-action="upgrade-pro">Start free trial</button>
            </div>
            <div class="settings-plan-card settings-plan-featured">
              ...
              <button class="btn btn-sm btn-accent" data-settings-action="upgrade-pro-plus">Start free trial</button>
            </div>
          </div>
        ` : `
          ...
          <div class="settings-section-row">
            ...
            ${!isProPlus ? `<button class="btn btn-sm btn-accent" data-settings-action="upgrade-pro-plus">Upgrade to Pro+</button>` : ""}
          </div>
        `}
      </div>
    ` : "";
      ...
      <div class="settings-panel-root" data-panel-type="settings">
        <div class="settings-panel-inner">
          ...
          <section class="settings-section">
            ...
            <div class="settings-pref-list">
              <label class="settings-pref-row">
                ...
                <button class="settings-toggle${state.compactMode ? " is-on" : ""}" data-pref-toggle="compactMode" type="button" aria-pressed="${state.compactMode ? "true" : "false"}">
          ...
          <footer class="settings-panel-footer">
            <small>Meridian Market Terminal · <a href="https://captainfredric.github.io/The-Terminal-Meridian/" target="_blank" rel="noopener">meridian.finance</a></small>
          </footer>
